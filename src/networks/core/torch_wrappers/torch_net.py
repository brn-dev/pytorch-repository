...
from src.torch_nn_modules import is_nn_activation_module, is_nn_dropout_module, is_nn_pooling_module, \
    is_nn_padding_module, is_nn_convolutional_module, is_nn_linear_module, is_nn_identity_module
...
class TorchNet(Net, Generic[ModuleType]):
    ...
    def detect_in_out_shapes(module: ModuleType):
        ...
        if is_nn_linear_module(module):
            in_shape, out_shape = TensorShape(features=module.in_features), TensorShape(features=module.out_features)

        elif is_nn_activation_module(module) or is_nn_dropout_module(module) or is_nn_identity_module(module):
        ...
        elif is_nn_padding_module(module):
            ...
            paddings = [sum(module.padding[2:]), sum(module.padding[:2])]

            for structural_name, padding in zip(out_shape.structural_dimension_names, paddings):
                out_shape[structural_name] = out_shape[structural_name] + padding
            ...
            if isinstance(num_features, (list, tuple, torch.Size)):
