...
class TinyModelDB(ModelDB[ModelInfo]):
    ...
    def __init__(
        ...
        tinydb.table.Table.document_id_class = str
    ...
    def fetch_entry(self, model_id: str) -> ModelEntry[ModelInfo]:
        return self.db.get(doc_id=self.get_doc_id(model_id))  # type: ignore
    ...
    def delete_entry(self, model_id: str, delete_state_dict: bool) -> None:
        ...
        removed_doc_ids = self.db.remove(doc_ids=[self.get_doc_id(model_id)])  # type: ignore
    ...
    def _save_entry(self, entry: ModelEntry[ModelInfo]):
        upserted_ids = self.db.upsert(
            tinydb.table.Document(entry, doc_id=self.get_doc_id(entry['model_id']))  # type: ignore
        )
